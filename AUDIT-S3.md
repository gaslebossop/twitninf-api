...
**À ce stade : 17 constats, dont 10 critiques et 2 élevés.** Les
neuvième et dixième constats critiques se distinguent des huit premiers :
aucun des deux ne demande de compte, de jeton, ni de connaissance du
système économique interne — seulement l'URL de la route — pour un impact
qui touche soit la disponibilité de la plateforme entière, soit un coût
facturé direct auprès d'un fournisseur tiers, plutôt qu'un seul compte ou
un seul solde. Le sixième
...
## Constat critique (10/10) — détail (décompte uniquement ici)

Une route ne porte aucune authentification et accepte, sans aucune
vérification qu'ils proviennent d'une recherche réelle, des tableaux de
données entièrement fournis par le client. Une garde minimale sur le
nombre total d'éléments de ces tableaux est trivialement satisfaite avec du
contenu fabriqué. Ces données alimentent ensuite un appel à un service
d'intelligence artificielle tiers facturé à l'usage, via une clé
d'API réelle. Le mécanisme de cache censé limiter les appels répétés ne
porte que sur le terme de recherche, pas sur le contenu fourni : varier
légèrement ce terme suffit à forcer un nouvel appel facturé à chaque fois.
C'est un vecteur de coût direct pour la plateforme, déclenchable sans
compte et cumulable indéfiniment.
